...
package io.kamax.hboxd.hypervisor.vbox6_1.data;
...
public class Mappings {
    ...
    public static void load() {
        ...
        log.debug("Loading Virtualbox Mappings");

        mappingMouse = new BiEnumMap<>(MouseMode.class, PointingHIDType.class);
        mappingMachine = new BiEnumMap<>(MachineStates.class, MachineState.class);

        mappingMachineSession = new BiEnumMap<>(MachineSessionStates.class, SessionState.class);
        mappingKeyboard = new BiEnumMap<>(KeyboardMode.class, KeyboardHIDType.class);
        mappingFirmware = new BiEnumMap<>(Firmware.class, FirmwareType.class);
        mappingAudioDriver = new BiEnumMap<>(AudioDriver.class, AudioDriverType.class);
        mappingAudioController = new BiEnumMap<>(AudioController.class, AudioControllerType.class);
    ...
    public static MachineStates get(MachineState type) {
        if (mappingMachine.containsReverse(type)) {
            ...
            log.error("Unable to find a Machine State mapping for " + type);
