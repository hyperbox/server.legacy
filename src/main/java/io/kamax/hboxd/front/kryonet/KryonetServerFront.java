...
public class KryonetServerFront implements _Front {
    ...
    public void start(_RequestReceiver r) throws HyperboxException {
        ...
        try {
            ...
            log.info("Kryonet connector is listening on port " + port);
    ...
    private void loadConfig() throws HyperboxException {
        try {
            port = Integer.parseInt(Configuration.getSetting(KryonetDefaultSettings.CFGKEY_KRYO_NET_TCP_PORT,
                    KryonetDefaultSettings.CFGVAL_KRYO_NET_TCP_PORT));
            log.debug("Found valid value for config key [" + KryonetDefaultSettings.CFGKEY_KRYO_NET_TCP_PORT + "]: " + port);
    ...
    @Override
    public void stop() {
        log.debug("Kryonet: stopping");
        server.stop();
        log.debug("Kryonet: stopped");
    }
    ...
    private class MainListener extends Listener {
        ...
        public void connected(Connection connection) {
            log.info("Conn #" + connection.getID() + " " + connection.getRemoteAddressTCP().getAddress().getHostAddress() + " connected.");
        ...
        public void received(Connection connection, Object object) {
            if (object.getClass().equals(Request.class)) {
                ...
                log.debug("Received request from " + client.getId() + " (" + client.getAddress() + ") : " + req.getExchangeId() + " - " + req.getCommand()
                        + " - " + req.getName());
        ...
        public void disconnected(Connection connection) {
            log.info("Conn #" + connection.getID() + " has disconnected.");
