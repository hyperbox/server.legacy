...
public abstract class HyperboxTest {
    ...
    protected final List<_RawVM> machines = new ArrayList<>();
    ...
    @After
    public void after() {
        List<_RawVM> deletedVms = new ArrayList<>();
        for (_RawVM vm : machines) {
            server.getHypervisor().deleteMachine(vm.getUuid());
            deletedVms.add(vm);
        }
        for (_RawVM vm : deletedVms) {
            machines.remove(vm);
            assertFalse(machines.contains(vm));
        }
